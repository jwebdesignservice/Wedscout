...
export default function EditVendorPage() {
  ...
  useEffect(() => {
    if (vendorId) {
      ...
      if (vendor) {
        ...
        setYearsExperience(vendor.years_experience);
        setWeddingsDone(vendor.weddings_done);
        setResponseTime(vendor.response_time);
        setInstagram(vendor.instagram || "");
        setTiktok(vendor.tiktok || "");
        setWebsite(vendor.website || "");
        setProfileImage(vendor.profile_image || "");
        setGalleryImages(vendor.gallery_images || []);
        setBio(vendor.bio?.length ? vendor.bio : [""]);
        setServices(vendor.services?.length ? vendor.services : [""]);
        if (vendor.pricing_tiers?.length) {
          setPricingTiers(vendor.pricing_tiers.map(t => ({
            name: t.name,
            price: t.price,
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      const updatedVendor: DemoVendor = {
        id: vendorId,
        name: name.trim(),
        category,
    ...
    <div className="min-h-screen bg-[#FFF4E2]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <h1 className="font-heading text-3xl font-light text-[#1A1A1A] mb-8">
          Edit <span className="italic">{name || "Vendor"}</span>
        </h1>
